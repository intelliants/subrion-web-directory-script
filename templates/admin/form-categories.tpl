...
intelli.cid = {$id|default:0}
        ...
        {capture name='title' append='field_after'}
            <div id="slug" class="row">
                <label for="field_categ_slug" class="col col-lg-2 control-label">{lang key='slug'} <a href="#" class="js-tooltip" title="{$tooltips.slug_literal}"><i class="i-info"></i></a></label>
                <div class="col col-lg-4">
                    <input type="text" name="slug" id="field_categ_slug" value="{if isset($item.slug)}{$item.slug}{/if}">
                    <p class="help-block text-break-word">{lang key='page_url_will_be'}: <span class="text-danger" id="title_url">{$smarty.const.IA_URL}{if isset($item.slug) && isset($parent.slug)}{$parent.slug}{$item.slug}/{/if}</span></p>
                </div>
            </div>
        {/capture}
