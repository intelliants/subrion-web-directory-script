...
final class PageRank
{
	 ...
	 */
	private static $_host = 'toolbarqueries.google.com';
	...
	public function getPageRank($q, $context = null)
	{
		...
		$url = sprintf($url, self::$_host, $ch, $q);

		$response = file_get_contents($url, false, $context);

		return $response ? end(explode(':', $response)) : 0;
	}
	...
	private function _makehash($string)
	{
		$check1 = self::_strtonum($string, 0x1505, 0x21);
		$check2 = self::_strtonum($string, 0, 0x1003f);
		$check1 >>= 2;
	...
	private function _checksum($hashnum)
	{
		$checkbyte = 0;
		$flag = 0;
